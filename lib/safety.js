  ...
  return drivers
    ...
    .map((driver) => {
      ...
      return {
        ...
        message:
          type === 'device_offline'
            ? `${driver.name || driver.phone} has not sent location updates for ${Math.max(2, Math.round(ageMs / 60000))} min.`
            : `${driver.name || driver.phone} is offline or has stopped tracking.`,
